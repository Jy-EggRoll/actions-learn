...
if ($Monitor) {
    ...
    Write-Host "您正在持续监测功率，使用 Ctrl + C 退出。`n`n请注意：功率监测严格根据电量变化计算，在脚本启动时或状态变化（如由放电转为充电）时的一次功率计算通常是不准确的。此脚本的电量直接从 Win32_Battery 获取，可能与托盘中显示的电量有微小差异，请以脚本为准。" -ForegroundColor Yellow
    Write-Host "---" -ForegroundColor DarkGray
